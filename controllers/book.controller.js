...
exports.getByIdBook = async (request, response) => {
    /** define id admin */
    let idBook = request.params.id
    
    /** call findAll() to get all data */
    let book = await adminModel.findAll({ where: { id: idBook } })
    return response.json({
        success: true,
        data: book,
        message: `Book data have been loaded`
    })
}
...
exports.updateBook = async (request, response) => {
    ...
    upload(request, response, async error => {
        ...
        bookModel.update(dataBook, { where: { id: id } })
            .then(result => {
